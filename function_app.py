...
import os                       # Built-in Python library for working with environment variables
...
from azure.cosmos import CosmosClient # Azure Cosmos DB SDK - for connecting to our database
...
def TextAnalyzer(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if text:
        ...
        # Generate a unique ID for this record using uuid4() 
        record_id = str(uuid.uuid4())

        # Create a document to store in Cosmos DB with the analysis results and metadata
        document = {
            "id": record_id,
        ...
        # Get the Cosmos DB container and create a new item (document) with our analysis results
        container = get_cosmos_container()
        container.create_item(body=document)

        response_data["recordId"] = record_id
...
# =============================================================================
# This endpoint retrieves previous text analysis records from Cosmos DB
@app.route(route="GetAnalysisHistory", methods=["GET"])
def GetAnalysisHistory(req: func.HttpRequest) -> func.HttpResponse:
    """
    This function retrieves past text analysis results from Cosmos DB.

    Query Parameter:
        limit: Optional. Maximum number of records to return. Default is 10.

    Returns:
        func.HttpResponse: JSON response with stored analysis history
    """
